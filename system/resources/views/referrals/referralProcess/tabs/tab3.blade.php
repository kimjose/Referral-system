...
@section('tab-content')
    <div class="tab-pane {{ $activeTab === 'tab3' ? 'active' : '' }}" id="tab3" role="tabpanel">
                    ...
                    <div class="card">
                        <div class="p-4">
                            <div class="pb-1">
                                ...
                                <p>Referring Officer: <span class="fs-5"> @if($referral != null) <strong>
                                {{ $referral->referringOfficer }}</strong> @endif </span></p>
                                <p>History/Investigation: <span class="fs-5"
                                        id="summary-historyInvestigation"><strong>{{ $referral->historyInvestigation }}</strong></span>
                                </p>
                                ...
                                <p>Reason for referral: <span class="fs-5"
                                        id="summary-reasonReferral"><strong>{{ $referral->reasonReferral }}</strong></span>
                                </p>
                                <p>Attachments: <span class="fs-5" id="summary-formFileMultiple"><strong></strong></span>
                                </p>
                                <p>Additional Notes: <span class="fs-5"
                                        id="summary-additionalNotes"><strong>{{ $referral->additionalNotes }}</strong></span>
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="p-4">
                            <div class="pb-1">
                                ...
                                <p>Priority Level: <span class="fs-5"
                                        id="summary-priorityLevel"><strong>{{ ucfirst($referral->priorityLevel) }}</strong></span>
                                </p>
                                {{-- <p>Service Category: <span class="fs-5" id="summary-serviceCategory"><strong>{{
                                            $referral->reasonReferral }}</strong></span></p>--}}
                                <p>Service: <span class="fs-5"
                                        id="summary-service"><strong>{{ $referral->service }}</strong></span></p>
                                <p>Facility: <span class="fs-5"
                                        id="summary-facility"><strong>{{ $referral->referredFacility }}</strong></span></p>
                                <p>Approximate Distance: <span class="fs-5"
                                        id="summary-distance"><strong>{{ $referral->distance }}</strong></span></p>
                                <p>Additional Notes: <span class="fs-5"
                                        id="summary-serviceNotes"><strong>{{ $referral->additionalNotes }}</strong></span>
                                </p>
                            </div>
                        </div>
                    </div>
        ...
        {{-- <div class="d-flex justify-content-end">--}}
            {{-- <button id="complete_button" class="btn btn-primary btn-lg">complete <i
        ...
        <div class="d-flex justify-content-end">
            <form action="{{route('referral.tabs.save', ['tab' => 'tab3'])}}" method="POST">
                @csrf
                <input type="hidden" id="user_id" name="user_id" value="{{ auth()->id() }}">
                <input type="hidden" id="patient_id" name="patientId" value="{{ $patient->id }}">
                <input type="hidden" id="referral_id" name="referralId"
                       value="{{ $referral->id}}">

                <input type="submit" value="Complete" class="btn btn-primary btn-lg">
            </form>
            <button id="complete_button" class="btn btn-primary btn-lg d-none" onclick="completeAction()">Complete <i
                    class="fas fa-check ml-2"></i></button>
